...
class SimplierInputAgent(AgentBase):
    def __init__(
        self,
    ...
    def _sampled_outputs_to_action_dict(sampled_outputs: dict[str, Tensor], state: Observations) -> dict[str, dict[str, int]]:
        ...
        return {
            ...
            "Attack": {
                ...
                else SimplierInputAgent._get_attack_target_index(attack_target_pos[0], attack_target_pos[1], state)
            }
        }
    ...
    def get_actions(
        ...
        for agent_id, observations in states.items():
            ...
            actions[agent_id] = ActionData(
                SimplierInputAgent._sampled_outputs_to_action_dict(agent_actions, observations),
                distributions,
                agent_actions,
                log_probs
            )
    ...
    def update(
        ...
        all_actions: dict[str, list[Tensor]] = {type: [] for type in SimplierNetwork.action_types()}
    ...
    def save(self, name: str | None = None) -> None:
        agent_name = name if name is not None else "SimplierInputAgent"
    ...
    @staticmethod
    def load(name: str | None = None) -> 'SimplierInputAgent':
        agent_name = name if name is not None else "SimplierInputAgent"
        jar = Jar("saved_agents")
        ...
        agent = SimplierInputAgent(**constructor_params)

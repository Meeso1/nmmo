...
class AttentionNetwork(nn.Module):
    def __init__(self, input_dim: int, output_dim: int = 128, attention_dim: int = 64):
        """
        Args:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        mask = (x.sum(dim=-1) != 0).float().unsqueeze(-1)  # (batch_size, ..., 1)
        
        # Compute attention scores
        attention_features = torch.tanh(self.attention_projection(x))
        scores = self.attention_scorer(attention_features)  # (batch_size, ..., 1)
        
        # Apply mask to scores
        scores = scores * mask - 1e9 * (1 - mask)
        attention_weights = nn.functional.softmax(scores, dim=1)  # (batch_size, ..., 1)
        
        # Transform input vectors
        features = self.feature_projection(x)  # (batch_size, ..., attention_dim)
